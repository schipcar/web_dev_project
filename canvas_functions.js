...
let db = new sqlite3.Database('./canvas.db', (err) => {
  ...
  console.log("test")
  db.all("SELECT * FROM announcements WHERE subject IN (SELECT announcement_subject FROM courses_announcements WHERE course_name = ?)", [localStorage.getItem("course_name")], function(err, row) {
